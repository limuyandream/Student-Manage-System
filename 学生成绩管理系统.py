...
def  Statistics():
    ...
    if os.path.exists(file_name):
        # 打开文件，读取数据
        with open(file_name, 'r',encoding='utf-8') as file:
            student_list = file.readlines()  # 字符串
            if student_list:
                print("一共有%d名学生"% len(student_list))
                # Statistics_total()
                extremum()
...
def Statistics_total():
    if os.path.exists(file_name):
        ...
        print("没有信息！")
    students.sort(key=lambda x:int(x['score_CN'])+int(x['score_MATH']),reverse=True)
    format_title = '{:^10}\t{:^12}\t{:^9}\t{:^11}\t{:^8}\t{:^10}'
    print(format_title.format('学号','姓名','语文成绩','数学成绩','总成绩','平均分'))
    for dic in students:
        format_data = '{:^10}\t{:^12}\t{:^12}\t{:^12}\t{:^12}\t{:^12}'
        total = int(dic.get('score_CN'))+int(dic.get('score_MATH'))
    ...
    if os.path.exists(file_name):
        ...
        while True:
            ...
            if key == 1:
                ...
                for i in students:
                    math_score.append(i.get('score_MATH'))
                math_max = max(math_score)
                format_title = '{:^10}\t{:^12}\t{:^11}'
                print("**************************")
                print("数学成绩最高分的同学为：")
                print(format_title.format('学号','姓名','数学成绩'))
                for i in students:
                    if i.get('score_MATH')==math_max:
                        format_data = '{:^10}\t{:^12}\t{:^12}'
                ...
                for i in students:
                    CN_score.append(i.get('score_CN'))
                CN_max = max(CN_score)
                format_title = '{:^10}\t{:^12}\t{:^11}'
                print("**************************")
                print("语文成绩最高分的同学为：")
                print(format_title.format('学号','姓名','语文成绩'))
                for i in students:
                    if i.get('score_CN')==CN_max:
                        format_data = '{:^10}\t{:^12}\t{:^12}'
                        print(format_data.format(i.get('id'),
                                                i.get('name'),
                                                i.get('score_CN'),
